<x-app-layout>
    <x-slot name="title">HSE Awards | Edit Role</x-slot>
    <x-slot name="header">
                ...
                <form action="{{ route('roles.update', $role) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                @foreach($permissions as $perm)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" class="mr-2" {{ in_array($perm->id, $rolePermissions) ? 'checked' : '' }}>
                                        {{ $perm->name }}
                                    </label>
                                @endforeach
                            </div>
